...
const getZgrade = (page = 0) => axios.get(`/zgrade?page=${page}&size=5`);
...
const AdminZgrade = () => {
    ...
    const {
        ...
        isPreviousData,
        data
    } = useQuery(['zgrade', page], () => getZgrade(page), { keepPreviousData: true });

    const onDelete = async (id) => {
        await axios.delete(`/zgrade/${id}`)
        queryClient.invalidateQueries('zgrade');
    }
    ...
    return (
        ...
        <div className="container mx-auto px-4 sm:px-8 max-w-3xl">
            <h1 className="text-gray-500 text-3xl mt-10 text-center font-light">Zgrade</h1>
            <button type="button" className="py-2 px-4  bg-purple-600 hover:bg-purple-700 focus:ring-purple-500 focus:ring-offset-purple-200 text-white transition ease-in duration-200 text-center text-base font-semibold rounded-lg shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 "
                onClick={() => history.push("/admin/zgrade/new")}>
                Dodaj zgradu
            </button>
            <div className="py-8">
